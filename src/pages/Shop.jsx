...
const Shop = () => {
  ...
  >
        ...
        {(category !== "all" || search) && <div className="flex flex-wrap items-center gap-2 mb-6">
            ...
            {category !== "all" && <span className="inline-flex items-center gap-1 bg-white/10 px-3 py-1 rounded-full text-[clamp(0.65rem,2cqw,0.75rem)] font-medium capitalize">
                Category: {category}
                <button onClick={() => setCategory("all")} className="ml-1 hover:text-red-400"><X className="w-3 h-3" /></button>
              </span>}
            {search && <span className="inline-flex items-center gap-1 bg-white/10 px-3 py-1 rounded-full text-[clamp(0.65rem,2cqw,0.75rem)] font-medium">
                Search: "{search}"
  ...
  >
        ...
        {filteredProducts.length > 0 ? <div className="grid grid-cols-1 min-[460px]:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 px-10 min-[460px]:px-0 gap-x-[clamp(0.6rem,1.5cqw,0.85rem)] gap-y-[clamp(1rem,2cqw,1.25rem)]">
            {filteredProducts.map((product) => <ProductCard key={product.id} product={product} />)}
          </div> : <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-[clamp(1.5rem,4cqw,2rem)] p-[clamp(2rem,6cqw,4rem)] text-center flex flex-col items-center justify-center shadow-2xl">
