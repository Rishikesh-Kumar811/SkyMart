...
export const Home: React.FC = () => {
  ...
  const topRated = useMemo(() =>
    [...PRODUCTS].sort((a, b) => b.rating.rate - a.rating.rate).slice(0, 5), []);
  const newArrivals = useMemo(() => PRODUCTS.slice(0, 5), []);
  const ProductMiniList = ({ products, title, icon: Icon, sortParam }: { products: typeof topRated, title: string, icon: any, sortParam?: string }) => (
  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto px-[clamp(1rem,4vw,9.5rem)] pt-[clamp(2rem,4vw,3rem)] pb-[clamp(2rem,5vw,4rem)]">
      <SEO 
        title="Home" 
      ...
      <section className="mb-10">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,clamp(120px,15cqw,180px)),1fr))] gap-[clamp(0.5rem,1.5cqw,1rem)]">
          {CATEGORIES.map(cat => (
            ...
            >
              <div className="text-[clamp(1.5rem,4cqw,2.5rem)] mb-[clamp(0.5rem,1cqw,1rem)] drop-shadow-md">{categoryIcons[cat] || '📦'}</div>
