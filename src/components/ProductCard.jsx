...
const ProductCard = ({ product }) => {
  ...
  />}
    ...
    className={cn(
      ...
      isInCart ? "bg-green-500/15 text-green-400 border border-green-500/20" : "bg-volt text-ink hover:bg-[#e2ff66]"
    )}
  >
            {isInCart ? <><Check className="w-[clamp(0.8rem,3.5cqw,1.2rem)] h-[clamp(0.8rem,3.5cqw,1.2rem)]" /> Added</> : <><ShoppingCart className="w-[clamp(0.8rem,3.5cqw,1.2rem)] h-[clamp(0.8rem,3.5cqw,1.2rem)]" /> Add</>}
          </button>
        </div>
      </div>
    </Link>;
