...
const Navbar = () => {
  ...
  return <nav id="main-navbar" className={cn(
  ...
  >
          ...
          <div className="flex items-center gap-[clamp(0.25rem,1cqw,0.5rem)] -mr-0.5">
            {user && <div className="hidden sm:flex items-center gap-[clamp(0.5rem,2cqw,0.75rem)] bg-white/[0.03] border border-white/10 p-[clamp(0.125rem,0.5cqw,0.25rem)] pr-[clamp(0.75rem,2cqw,1rem)] rounded-[clamp(0.75rem,2cqw,1rem)] backdrop-blur-sm shadow-inner transition-colors duration-300 ease-apple hover:bg-white/5 cursor-default">
  ...
  >
              ...
              {cartCount > 0 && <span className="absolute -top-1 -right-1 w-[clamp(1rem,2.5cqw,1.25rem)] h-[clamp(1rem,2.5cqw,1.25rem)] bg-volt text-ink rounded-full flex items-center justify-center text-[clamp(0.5rem,1.2cqw,0.6rem)] font-bold border-[2px] border-[#0d0d0d] shadow-[0_0_10px_rgba(200,244,0,0.3)]">
                  {cartCount}
                </span>}
            </button>
            {user ? <button
  ...
  >
    ...
    className={({ isActive }) => cn(
      ...
      isActive ? "bg-volt/10 text-volt border border-volt/20 shadow-[0_0_15px_rgba(200,244,0,0.15)]" : "text-white/60 hover:bg-white/5 hover:text-white border border-transparent"
    )}
  >
              {link.label}
              <span className={cn(
