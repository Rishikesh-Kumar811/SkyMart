...
const ProductDetail = () => {
  ...
  return <div className="w-full max-w-[1600px] mx-auto px-[clamp(1rem,4vw,9.5rem)] pt-[clamp(2rem,4vw,3rem)] pb-[clamp(1.5rem,4vw,3rem)]">
      <SEO
    title={product.title}
  ...
  />
      ...
      <div className="grid lg:grid-cols-2 gap-[clamp(1.5rem,5cqw,3rem)] mb-[clamp(2rem,6cqw,4rem)]">
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-[clamp(1.5rem,4cqw,2rem)] flex items-center justify-center aspect-square overflow-hidden shadow-xl">
          {imgError ? <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-volt/10 to-volt/25 rounded-xl gap-3">
            ...
            </div> : <img src={product.image} alt={product.title} onError={() => setImgError(true)} className="w-full h-full object-contain p-[clamp(2rem,6cqw,4rem)] mix-blend-screen drop-shadow-2xl filter contrast-125" />}
        </div>
    ...
    className={`flex-1 flex items-center justify-center gap-2 py-[clamp(0.75rem,3cqw,1rem)] rounded-[clamp(0.75rem,3cqw,1rem)] font-heading font-bold text-[clamp(0.875rem,2.5cqw,1rem)] transition-all cursor-pointer active:scale-[0.97] shadow-lg hover:shadow-xl ${isInCart ? "bg-green-500/15 text-green-400 border border-green-500/20 backdrop-blur-sm" : "bg-volt text-ink hover:bg-[#e2ff66] shadow-[0_0_20px_rgba(200,244,0,0.2)]"}`}
  >
    ...
    className={`w-[clamp(3rem,10cqw,3.5rem)] h-[clamp(3rem,10cqw,3.5rem)] rounded-[clamp(0.75rem,3cqw,1rem)] border flex items-center justify-center transition-colors cursor-pointer backdrop-blur-sm shadow-lg ${liked ? "bg-red-500/15 border-red-500/30 text-red-400 shadow-[0_0_15px_rgba(239,68,68,0.2)]" : "bg-white/5 border-white/10 text-white/40 hover:text-red-400 hover:border-red-400/30"}`}
  >
