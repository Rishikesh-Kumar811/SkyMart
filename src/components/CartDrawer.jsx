...
const CartDrawer = () => {
    ...
    isCartOpen ? "translate-x-0" : "translate-x-full"
        ...
        <div className="p-[clamp(1rem,3cqw,1.5rem)] border-b border-white/10 flex items-center justify-between bg-white/5">
          <h2 className="text-[clamp(1.25rem,4cqw,1.5rem)] font-heading font-bold flex items-center gap-[clamp(0.5rem,2cqw,0.75rem)] drop-shadow-md">
            ...
            Cart
            {cart.length > 0 && <span className="text-[clamp(0.65rem,2cqw,0.75rem)] bg-volt/20 border border-volt/30 text-volt px-[clamp(0.5rem,2cqw,0.75rem)] py-[clamp(0.125rem,0.5cqw,0.25rem)] rounded-full font-bold shadow-[0_0_15px_rgba(200,244,0,0.1)]">
  ...
  >
        ...
        <div className="flex-grow overflow-y-auto p-[clamp(1rem,3cqw,1.5rem)] space-y-[clamp(1rem,3cqw,1.5rem)] scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
          {cart.length === 0 ? <div className="h-full flex flex-col items-center justify-center text-center space-y-[clamp(0.75rem,2cqw,1rem)]">
  ...
  >
            ...
            </div> : cart.map((item) => <div key={item.product.id} className="flex gap-[clamp(0.75rem,3cqw,1rem)] bg-white/5 backdrop-blur-md rounded-[clamp(1rem,3cqw,1.5rem)] p-[clamp(0.75rem,2.5cqw,1rem)] border border-white/10 shadow-lg">
  ...
  >
        ...
        {cart.length > 0 && <div className="p-[clamp(1rem,3cqw,1.5rem)] border-t border-white/10 bg-white/5 backdrop-blur-md">
            ...
            <button
    onClick={() => {
      showToast("Order placed! \u{1F386} (Demo)");
      setIsCartOpen(false);
      setTimeout(() => {
